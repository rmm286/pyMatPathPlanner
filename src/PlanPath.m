function [stateOut,controlOut] = PlanPath(start_pt, goal_pt)
...
start_pt = cell(start_pt);
goal_pt = cell(goal_pt);

start_pt = cellfun(@double,start_pt);
goal_pt = cellfun(@double,goal_pt);
...
xf = goal_pt(1);
yf = goal_pt(2);
thf = goal_pt(3); %note, solver is sensitive to positive or negative theta
kf = goal_pt(4);
kDotf = goal_pt(5);
vf = goal_pt(6);
vDotf = goal_pt(7);
